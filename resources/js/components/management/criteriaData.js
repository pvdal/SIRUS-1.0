// Funções auxiliares que você provavelmente já tem em algum lugar
// Se não, você pode criá-las ou adaptar o código.
...
export function criteriaData() {
    return {
        ...
        searchTerm: '',
        statusFilter: { value: '', name: '', drop: false },
        registerPeriod: { value: '', name: '', drop: false },
        ...
        // Carrega os dados dos critérios (com filtros)
        async loadCriteria(page = 1) {
            this.loading = true;
            document.body.style.cursor = 'wait'; // Muda o cursor
            this.errors = {};
            this.newCriteria = [];
        ...
        async saveCriterion() {
            ...
            if (isUpdate && this.criterionId) {
                url = `/criteria/${this.criterionId}/update`; // Rota para atualizar
                method = 'put';
            }
        ...
        async toggleStatus(id = null) {
            ...
            const action = criterion.state === 1 ? 'inactivate' : 'activate';
            const idList = action === 'inactivate' ? this.inactivatingIds : this.activatingIds;
        ...
        clearFields(type) {
            clearComponentData(this, type,
                ['name', 'description'], // Campos do formulário
                [], // Não há filtros complexos para limpar
            );
        },
