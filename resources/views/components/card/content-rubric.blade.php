        ...
        @if($axes)
            @if($axesCount)
                <div class="flex items-center gap-1 text-sm text-gray-700 dark:text-gray-300 font-medium px-6 pb-2 flex-shrink-0 transition duration-150 ease-in-out">
                    <x-lucide-clipboard-list class="w-4 h-4 text-gray-500 dark:text-gray-400 transition duration-150 ease-in-out" />
                    <span
                        x-text="`${item.{{ $axesCount }}.length} ${item.{{ $axesCount }}.length === 1 ? 'Eixo' : 'Eixos'}`">
                    </span>
                </div>
            @endif
