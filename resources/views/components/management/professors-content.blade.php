<div>
    ...
    <div class="py-5 px-2">
        ...
        <table class="min-w-full border border-gray-300 divide-y divide-gray-200 rounded-sm">
            ...
            <template x-for="professor in [...newProfessors, ...professors]" :key="professor.id">
                <tr :class="
                    {
                        'hover:bg-gray-50': true,
                        'bg-green-50': professor.origin === 'new',
                    }"
                    x-transition:enter="transition ease-out duration-500"
                    x-transition:enter-start="opacity-0 transform scale-95 -translate-y-2"
                    x-transition:enter-end="opacity-100 transform scale-100 translate-y-0"
                >
                    <td class="px-4 py-2 border text-center border-gray-300 hidden md:table-cell" x-text="professor.id"></td>
                    <td class="px-4 py-2 border text-center border-gray-300" x-text="professor.name"></td>
                    <td class="px-4 py-2 border text-center border-gray-300 hidden sm:table-cell" x-text="professor.email"></td>
                    <td class="px-4 py-2 border text-center border-gray-300 hidden sm:table-cell" x-text="professor.state ? 'Ativo' : 'Inativo'"></td>
                    <td class="px-4 py-2 border text-center border-gray-300">
                        <x-button type="button" class="min-w-[98px]"
                            x-on:click="
                                showProfessor(professor.user_id);
                                $el.blur();
                            "
                        >
                            Alterar
                        ...
                        <template x-if="professor.state">
                            <x-danger-button type="button" class="min-w-[98px]" x-bind:disabled="isInactivating(professor.user_id)"
                                ...
                                "
                            >
                                <template x-if="isInactivating(professor.user_id)">
                        ...
                        <template x-if="!professor.state">
                            <x-management.activate-button type="button" class="min-w-[98px]" x-bind:disabled="isActivating(professor.user_id)"
                                x-on:click="
                                    activate(professor.user_id);
                                    $el.blur();
                                "
                            >
                                <template x-if="isActivating(professor.user_id)">
                                    <span>Ativando...</span>
                                </template>
                                <template x-if="!isActivating(professor.user_id)">
                                    <span>Ativar</span>
    ...
    <template x-if="empty">
        <div class="flex flex-col items-center justify-center pb-8 pt-2 text-center">
            <p class="text-gray-700 text-md font-medium">
                Nenhum professor foi cadastrado até o momento.
            </p>
            <p class="text-gray-500 mt-1 text-sm">
                Assim que houverem professores registrados, eles aparecerão aqui.
            </p>
        </div>
    </template>
</div>
