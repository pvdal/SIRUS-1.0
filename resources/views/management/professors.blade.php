        ...
        <div x-data="professorsData()"
            ...
            <div x-show="showImportModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div @click.outside="showImportModal = false" class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-2xl max-w-md w-full">
                    <h2 class="text-2xl font-bold mb-4 text-gray-800 dark:text-white">Importar Professores</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
                        Para importar, utilize nosso modelo padrão para evitar erros de leitura.
                        <a href="{{ route('users.professors.download-template') }}" class="text-blue-500 font-bold block mt-2 underline">
                            Baixar Modelo Excel
                        </a>
                    </p>
                    <form action="{{ route('users.professors.import') }}" method="POST" enctype="multipart/form-data"
                          @submit="setTimeout(() => { loadProfessors(); showImportModal = false; }, 1000)">
                        @csrf
                        <div class="mb-4">
